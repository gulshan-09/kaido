...
search.get('/search/:id/:page?', async (req , res)=>{
    ...
    const searchlink = `https://hianime.to/search?keyword=${searchdetails}&page=${pagenumber}`;
    const searchlinkani = `https://hianime.to/search?keyword=${searchdetails}&page=${pagenumber +1}`;
   ...
   try {
    ...
    $('.flw-item').each((index,element)=>{
        ...
        const dubani = $(element).find('.tick-dub').text() || 0;
